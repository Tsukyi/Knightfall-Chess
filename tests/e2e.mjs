...
try{
 ...
 await check('mobile layouts fit screen without horizontal overflow',async()=>{
  for(const width of [390,768]){await page.setViewportSize({width,height:844});for(const view of ['play','learn','studio']){await page.locator(`.nav-item[data-route="${view}"]`).click();const overflow=await page.evaluate(()=>({fits:document.documentElement.scrollWidth<=innerWidth+1,offenders:[...document.querySelectorAll('body *')].filter(el=>el.getBoundingClientRect().right>innerWidth+1).slice(0,12).map(el=>({tag:el.tagName,cls:el.className,right:el.getBoundingClientRect().right}))}));assert.ok(overflow.fits,`${view} overflow at ${width}: ${JSON.stringify(overflow.offenders)}`);await page.screenshot({path:`artifacts/${view}-${width}.png`,fullPage:true});}}
 });
